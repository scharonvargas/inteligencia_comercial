...
const App: React.FC = () => {
  ...
  // Effect to persist results on change
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEYS.RESULTS, JSON.stringify(results));
    } catch (e) {
      console.warn("Storage full or error saving results", e);
    }
  }, [results]);
  ...
  const handlePipelineChange = useCallback(async (businessId: string, newStage: 'new' | 'contacted' | 'meeting' | 'closed' | 'lost') => {
  ...
  const handleClearCache = useCallback(() => {
    clearMemoryCache();
    localStorage.removeItem(STORAGE_KEYS.RESULTS); // Also clear saved results
