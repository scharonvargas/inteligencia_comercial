...
import { 
  ...
  ChevronLeft, ChevronRight, Mail, Sparkles, Copy, Loader2, Check, Star
} from 'lucide-react';
...
export const ResultsTable: React.FC<ResultsTableProps> = ({ data }) => {
  ...
  const handleToggleProspect = async (e: React.MouseEvent, business: BusinessEntity) => {
    e.stopPropagation(); // Impede que a linha expanda ao clicar na estrela
    
    // Atualização otimista da UI
    const newStatus = !business.isProspect;
    setLocalData(prev => prev.map(b => b.id === business.id ? { ...b, isProspect: newStatus } : b));
  ...
  const handleGenerateEmail = async (biz: BusinessEntity) => {
                ...
                paginatedData.map((biz) => (
                  <React.Fragment key={biz.id}>
                    <tr 
                      ...
                      className={`cursor-pointer transition-colors group ${expandedRowId === biz.id ? 'bg-slate-700/40' : 'hover:bg-slate-700/30'}`}
                    >
                      <td className="p-4 text-slate-500">
                        {expandedRowId === biz.id ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                      </td>
                      <td className="p-4 text-center">
                        <button 
                          onClick={(e) => handleToggleProspect(e, biz)}
                    ...
                    {/* Expanded Detail Row */}
                    {expandedRowId === biz.id && (
                      <tr className="bg-slate-700/20 animate-fadeIn">
                          ...
                          <div className="p-6 border-l-2 border-brand-500 ml-4 md:ml-0">
                            ...
                            <div className="bg-slate-800 rounded border border-slate-700 p-4">
                                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                                  ...
                                  {!generatedEmails[biz.id] && (
                                    <button 
                                      onClick={(e) => { e.stopPropagation(); handleGenerateEmail(biz); }}
                                      disabled={loadingEmailId === biz.id}
                                    ...
                                    >
                                      {loadingEmailId === biz.id ? <Loader2 className="animate-spin" size={12}/> : <Mail size={12} />}
                                      Gerar Rascunho
                                    </button>
                                  )}
                                </div>
                                ...
                                {generatedEmails[biz.id] ? (
                                  <div className="animate-fadeIn">
                                    ...
                                    <div className="mt-2 flex justify-end">
                                      <button
                                        onClick={(e) => { e.stopPropagation(); copyToClipboard(generatedEmails[biz.id], biz.id); }}
        ...
        {filteredData.length > 0 && (
          <div className="border-t border-slate-700 bg-slate-900/30 p-4 flex items-center justify-between">
            <div className="text-sm text-slate-500">
              Exibindo <span className="text-slate-300 font-medium">{startIndex + 1}</span> até <span className="text-slate-300 font-medium">{Math.min(startIndex + itemsPerPage, filteredData.length)}</span> de <span className="text-slate-300 font-medium">{filteredData.length}</span> resultados
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  // Logic to show pages around current, simplified for now
                  let pNum = i + 1;
